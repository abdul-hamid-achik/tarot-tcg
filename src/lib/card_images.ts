...
function mapCardNameToImagePath(cardName: string, cardId?: string): string | null {
  ...
  const minorPattern =
    /^(ace|one|two|three|four|five|six|seven|eight|nine|ten|page|knight|queen|king)\s+of\s+(cups|pentacles|swords|wands|coins)$/i
  ...
  if (cardId) {
    const idPattern =
      /^(major_\d{2}|(?:cups|pentacles|swords|wands)_(?:\d{2}|king|queen|knight|page))$/
