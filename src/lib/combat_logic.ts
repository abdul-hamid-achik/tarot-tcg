import { Card, GameState, PlayerId } from '@/schemas/schema'
...
export function getValidAttackTargets(
    ...
    const tauntUnits = enemyUnits.filter(unit =>
        unit.keywords?.includes('taunt') || unit.keywords?.includes('Taunt')
    )
 ...
 */
export function previewCombat(attacker: Card, defender: Card): {
