...
export class GameLogger {
  ...
  static gameStart(player1Name: string, player2Name: string) {
    console.log('%c🎯 === NEW GAME STARTED ===', 'color: #a855f7; font-size: 14px; font-weight: bold;');
  ...
  static turnStart(player: string, turn: number, round: number, hasAttackToken: boolean) {
    ...
    console.log(`Turn ${turn} | Round ${round} | Active: ${player} | Attack Token: ${hasAttackToken ? '⚔️' : '🛡️'}`);
  }

  static combatSummary(attackers: {name: string; attack: number; health: number}[], defenders: {name: string; attack: number; health: number}[], damage: number) {
    console.group('%c💥 Combat Resolution', 'color: #ef4444; font-weight: bold;');
    console.log('Attackers:', attackers.map(a => `${a.name} (${a.attack}/${a.health})`));
    console.log('Defenders:', defenders.map(d => `${d.name} (${d.attack}/${d.health})`));
