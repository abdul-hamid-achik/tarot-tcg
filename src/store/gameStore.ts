...
export const getCellType = (position: CellPosition): CellType => {
    switch (position.row) {
        case GRID_ROWS.ENEMY_BENCH: return 'enemy_bench'
        case GRID_ROWS.ENEMY_ATTACK: return 'enemy_attack'
        case GRID_ROWS.PLAYER_ATTACK: return 'player_attack'
        case GRID_ROWS.PLAYER_BENCH: return 'player_bench'
    }
}
    ...
    devtools(
        (set, get) => ({
            ...
            setValidDropZones: (positions: CellPosition[]) => {
                ...
                }), false, 'setValidDropZones')
            },

            clearValidDropZones: () => {
                set((state: GameStore) => ({
                    grid: { ...state.grid, validDropZones: new Set() }
                }), false, 'clearValidDropZones')
            },
            ...
            assignDefender: (laneId: number, defenderId: string) => {
                set((state: GameStore) => ({
                    interaction: {
                        ...state.interaction,
                        defenderAssignments: new Map(state.interaction.defenderAssignments).set(laneId, defenderId)
                    }
                }), false, 'assignDefender')
            },
            ...
            showCardDetail: (card: GameCard) => {
                ...
                }, false, 'showCardDetail')
            },

            hideCardDetail: () => {
                set((state: GameStore) => ({
                    ui: { ...state.ui, cardDetailOverlay: null, activeOverlay: 'none' }
                }), false, 'hideCardDetail')
            },

            setOverlay: (overlay: 'none' | 'cardDetail' | 'mulligan' | 'gameOutcome', data: unknown = null) => {
                set((state: GameStore) => ({
