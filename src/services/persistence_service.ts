...
export class PersistenceService {
  ...
  async saveGameState(sessionId: string, gameState: AppGameState): Promise<void> {
    try {
      ...
      GameLogger.debug(
        `Saved game state for session ${sessionId} (Turn ${gameState.turn}, Round ${gameState.round})`,
      )
    } catch (error) {
  ...
  async loadGameState(sessionId: string): Promise<AppGameState | null> {
    try {
      ...
      GameLogger.debug(
        `Loaded game state for session ${sessionId} (Turn ${latestState.turn}, Round ${latestState.round})`,
      )
