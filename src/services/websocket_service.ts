...
export class WebSocketService {
  ...
  private buildWebSocketUrl(gameId: string, playerId: PlayerId, token: string): string {
    const baseUrl =
      process.env.NODE_ENV === 'production' ? 'wss://your-app.vercel.app' : 'ws://localhost:3000'
...
export function useWebSocket() {
  return {
    ...
    playCard: (card: Card, targetSlot?: number) => webSocketService.playCard(card, targetSlot),
