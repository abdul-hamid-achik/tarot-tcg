import type { GameState, Card, Player } from '@/schemas/gameSchemas'
...
export class AIService {
  ...
  performMulligan(gameState: GameState): GameState {
    ...
    if (Math.random() < personality.mistakeChance) {
      ...
      if (Math.random() < 0.5) {
        ...
        const goodCards = hand.filter(card => 
          !cardsToMulligan.includes(card.id) && this.evaluateCardValue(card) > 0.7
        )
  ...
  selectCardToPlay(gameState: GameState): { card: Card | null, shouldPlay: boolean } {
    ...
    const playableCards = player.hand.filter(card => 
      card.cost <= (player.mana + player.spellMana)
    )
    ...
    switch (personality.playStrategy) {
      ...
      case 'tempo':
        ...
        selectedCard = playableCards
          .sort((a, b) => this.evaluateTempoValue(a, gameState) - this.evaluateTempoValue(b, gameState))[0]
        break

      case 'control':
        // Prefer value and card advantage
        selectedCard = playableCards
          .sort((a, b) => this.evaluateCardValue(b) - this.evaluateCardValue(a))[0]
        break
    }
    ...
    switch (personality.attackStrategy) {
      ...
      case 'cautious':
        ...
        const safeAttackers = availableAttackers.filter(attacker => {
          const wouldSurvive = opponentUnits.every(defender => 
  ...
  private evaluateHandQuality(hand: Card[]): number {
    ...
    const costVariance = hand.reduce((sum, card) => sum + Math.pow(card.cost - avgCost, 2), 0) / hand.length
  ...
  private calculateOptimalAttack(gameState: GameState, availableAttackers: Card[]): string[] {
    ...
    const goodAttackers = availableAttackers.filter(attacker => {
      const canKillDefender = opponentUnits.some(defender => 
        (attacker.attack || 0) >= (defender.health || 0)
      )
      const survivesCounterAttack = opponentUnits.every(defender =>
        (defender.attack || 0) < (attacker.health || 0)
      )
