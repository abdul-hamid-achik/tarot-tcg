...
import type { GameState, Card as GameCard, Lane, Player } from '@/schemas/gameSchemas'
...
class CombatService {
    ...
    private applyEffectToTargets(gameState: GameState, effect: PersistentEffect): GameState {
        ...
        const getTargetCards = (): GameCard[] => {
            switch (effect.targets) {
                ...
                case 'all_friendly':
                    const sourceCard = this.findCardInGame(newState, effect.sourceCardId)
     ...
     */
    private processKeywordAbilities(attacker: GameCard, defender: GameCard | null, laneIndex: number): CombatModifiers {
     ...
     */
    private registerReversedAbilities(card: GameCard, gameState: GameState): void {
        ...
        this.registerTriggeredAbility({
            ...
            effect: (context: GameEventContext): GameState => {
                ...
                if (context.gameState[opponent].spellMana > 0) {
                    context.gameState[opponent].spellMana = Math.max(0, context.gameState[opponent].spellMana - 1)
    ...
    private parseEnterBenchEffect(card: GameCard) {
        return (context: GameEventContext): GameState => {
            ...
            if (description.includes('gain') && description.includes('mana')) {
                ...
                context.gameState[context.player!].spellMana = Math.min(3,
                ...
                const manaMatch = description.match(/(\d+)\s*mana/)
                const mana = manaMatch ? parseInt(manaMatch[1]) : 1
                context.gameState[context.player!].spellMana = Math.max(0,
                    context.gameState[context.player!].spellMana - mana
                )
            }
    ...
    private parseBenchAuraEffect(card: GameCard): PersistentEffect['effect'] {
        ...
        if (description.includes('other units get +')) {
            ...
            if (statMatch) {
                effect.attackModifier = parseInt(statMatch[1])
                effect.healthModifier = parseInt(statMatch[2])
            }
        }
    ...
    private calculateCombatResolution(gameState: GameState): CombatResolution {
        ...
        const survivingAttackers = results
            .filter(r => r.attackerSurvived)
            .map(r => r.attacker)
     ...
     */
    private async animateCombatAttacks(gameState: GameState, resolution: CombatResolution): Promise<void> {
        const attackingPlayer = gameState.attackingPlayer!
     ...
     */
    private async applyDamageWithAnimations(gameState: GameState, resolution: CombatResolution): Promise<void> {
        const attackingPlayer = gameState.attackingPlayer!
        const defendingPlayer = attackingPlayer === 'player1' ? 'player2' : 'player1'
        ...
        resolution.results.forEach((result) => {
            ...
            if (result.attackerDamage > 0) {
                ...
                damageAnimations.push(
                    animationService.animateDamage(
                        attackerPosition,
    ...
    private applyUnitDeaths(gameState: GameState, resolution: CombatResolution): void {
        ...
        resolution.results.forEach((result) => {
            ...
            if (!result.attackerSurvived) {
                ...
                if (benchUnit) {
                    benchUnit.currentHealth = (benchUnit.currentHealth || benchUnit.health) - result.attackerDamage
            ...
            if (result.defender) {
                if (!result.defenderSurvived) {
                    ...
                    if (benchUnit) {
                        benchUnit.currentHealth = (benchUnit.currentHealth || benchUnit.health) - result.defenderDamage
            ...
            if (result.defenderDamage > 0) {
                this.handleLifelinkHealing(gameState, result.attacker, result.defenderDamage, attackingPlayer)
            }
        })
    }
     ...
     */
    private async animateUnitsReturning(gameState: GameState, resolution: CombatResolution): Promise<void> {
        const attackingPlayer = gameState.attackingPlayer!
        const defendingPlayer = attackingPlayer === 'player1' ? 'player2' : 'player1'
    ...
    private logCombatResults(resolution: CombatResolution): void {
        const combatResultsForLogging = resolution.results.map(result => ({
            ...
            defender: result.defender ? `${result.defender.name} (${result.defenderSurvived ? 'survived' : 'died'})` : null,
     ...
     * Handle lifelink healing
     */
    private handleLifelinkHealing(gameState: GameState, attacker: GameCard, damageDealt: number, attackingPlayer: 'player1' | 'player2'): void {
