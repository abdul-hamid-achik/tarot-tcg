...
function _getPlayerUnits(gameState: GameState, playerId: 'player1' | 'player2'): Card[] {
  const units =
...
export class StateManager {
  ...
  playCardFromHand(cardId: string, targetSlot?: number): boolean {
    ...
    if (card.type === 'unit') {
      const units =
        this.gameState.activePlayer === 'player1'
          ? this.gameState.battlefield.playerUnits
          : this.gameState.battlefield.enemyUnits

      // Find target slot
      const slot = targetSlot !== undefined ? targetSlot : units.indexOf(null)
