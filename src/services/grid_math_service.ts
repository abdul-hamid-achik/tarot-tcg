...
export class GridMathService {
  ...
  getCellBounds(position: CellPosition): DOMRect {
    const x =
      this.dimensions.gridLeft + position.col * (this.dimensions.cellWidth + this.dimensions.gap)
    const y =
      this.dimensions.gridTop + position.row * (this.dimensions.cellHeight + this.dimensions.gap)
  ...
  isValidPosition(position: CellPosition): boolean {
    return position.row >= 0 && position.row <= 3 && position.col >= 0 && position.col <= 5
  }
