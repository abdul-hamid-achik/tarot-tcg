...
export const useGameClock = (config: Partial<GameClockConfig> = {}) => {
  ...
  const [timeRemaining, setTimeRemaining] = useState(config.turnTimeLimit || DEFAULT_CONFIG.turnTimeLimit)
  ...
  }, [gameState?.turn, gameState?.activePlayer, gameState?.phase, mergedConfig])
