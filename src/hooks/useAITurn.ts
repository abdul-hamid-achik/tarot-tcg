...
import type { GameState } from '@/schemas/gameSchemas'
...
export const useAITurn = () => {
  ...
  // AI defender logic
  const executeAIDefense = useCallback((state: GameState): { defenderId: string; laneId: number }[] => {
    ...
    state.lanes.forEach((lane, index) => {
      if (lane.attacker && availableDefenders.length > 0) {
        const defender = availableDefenders
          .sort((a, b) => {
  ...
  const executeFullAITurn = useCallback(async () => {
    ...
    // Phase 1: Play cards
    let currentState = executeAICardPlay(gameState)
