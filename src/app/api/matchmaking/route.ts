...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const _gameMode = searchParams.get('gameMode') || 'standard'
    ...
    for (const [_mode, pool] of matchmakingPools) {
      pool.delete(playerId)
    }
...
function estimateWaitTime(poolSize: number, _gameMode: string): string {
