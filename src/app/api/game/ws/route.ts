...
import { GameState, PlayerId } from '@/schemas/schema'
...
async function registerConnection(gameId: string, playerId: PlayerId, socket: WebSocket): Promise<void> {
...
function getPlayerSide(gameId: string, playerId: PlayerId): PlayerId {
...
async function handlePlayCard(gameState: GameState, message: any, playerId: PlayerId): Promise<GameState> {
...
async function handleDeclareAttack(gameState: GameState, message: any, playerId: PlayerId): Promise<GameState> {
...
async function handleEndTurn(gameState: GameState, playerId: PlayerId): Promise<GameState> {
...
async function broadcastToOthers(gameId: string, excludePlayerId: PlayerId, message: any): Promise<void> {
    const session = gameSessions.get(gameId)
    if (!session) return
