...
export default async function CardPage({ params }: PageProps) {
  ...
  const card = allCards.find((card) => 
    card._raw.flattenedPath === `cards/${slug}`
  )
  ...
  const breadcrumbItems = [
    ...
    ...(slugArray.length > 1 ? [{ 
      label: slugArray[0].split('-').map(word => 
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-4">
          <h1 className="text-4xl font-bold text-black dark:text-white transition-colors">{card.name}</h1>
            ...
            <CardContent>
              {card.abilities.upright && (
                <div className="mb-4">
                  ...
                  {card.abilities.upright.map((ability: { name: string; description: string }, index: number) => (
              ...
              {card.abilities.reversed && (
                <div>
                  ...
                  {card.abilities.reversed.map((ability: { name: string; description: string }, index: number) => (
                    <div key={index} className="mb-2">
                      <strong>{ability.name}:</strong> {ability.description}
                    </div>
                  ))}
