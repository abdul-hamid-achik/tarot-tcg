    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      {showDetails && (
        <div className="flex gap-4 text-xs text-gray-400">
          {queuedMessages > 0 && <span>📦 {queuedMessages} queued</span>}
          {pendingActions > 0 && <span>⏳ {pendingActions} pending</span>}
          {multiplayer.connectionStatus === 'connected' && <span>🌐 Real-time</span>}
        </div>
      )}
    </div>
  )
...
export function GameJoinInterface({ onJoin, onCreateGame, className }: GameJoinProps) {
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-4 p-4 bg-gray-900/50 rounded-lg border border-gray-700',
        className,
      )}
    >
